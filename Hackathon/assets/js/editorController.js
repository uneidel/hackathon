app.controller('processDialogCtrl', function ($scope, $http,$interval, ngDialog, Config) {
    $scope.Workingstatus = [];
    ...
    if (Config.Url)
    {
        $scope.inputurl = Config.Url;
    }
});
app.controller("EditorController", function ($scope, $http, ngDialog, Config, azureBlob) {
    $scope.UploadTitle = "Upload File";
    ...
    $scope.open = function (file,errFiles) {
    ...
    $scope.uploadFiles = function () {
        ...
        $http.get('/api/config', { headers: { 'Accept': 'application/json' } }).then(function (result) {
            var data = result.data;
            var fullurl = data.ContainerUri + "/" + f.name + data.SasToken;
            if (f) {
               // r.readAsArrayBuffer(file);
                var config = {
                    baseUrl: data.ContainerUri + "/" + f.name,
                    sasToken: data.SasToken,
                    file: f,
                    progress: UploadProgress,
                    complete: UploadDone(fullurl),
                    error: null
                }
    ...
    function UploadProgress(result) {
        $scope.UploadTitle = result;
    }
